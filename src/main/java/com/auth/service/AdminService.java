...
public class AdminService {
	...
	@Autowired
	private AdminCurrentlyLoggedRepository admincurrRepo;
   ...
   //find all admins
   public List<String> allAdminsEmail(){
	   List<String> ans = adminAllDetail.allAdminsEmail();
	   return ans;
   }
   
   //find who is currently logged admin
   public String select() {
	   return admincurrRepo.findCurrentAdmin().get(0).getRegisterEmail();
   }
   
   public List<AdminCurrentlyLogged> selectAll() {
	   return admincurrRepo.findCurrentAdmin();
   }
   
 //find who is currently logged admin dob
   public String selectDob(String email) {
	   return adminAllDetail.getDob(email);
   }
   
   //updating changed entry from AdminAllDetailRepository adminAllDetail;
   public int updateAdminDetailFromAllDetailRepo(String email,String name,String dob) {
	   int ans = adminAllDetail.updateEntry(email, name, dob);
	   return ans;
   }
   //updating changed entry from AdminLoginRepository adminLoginRepo;
   public int updateAdminDetailFromLoginRepo(String email,String name) {
	   int ans = adminLoginRepo.updateEntry(email, name);
	   return ans;
   }
